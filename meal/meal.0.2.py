...
def validate_12_hour_format(time):
    ...
    return len(time) == 8 and time[2] == ":" and time[5] == " " and (time[:2].isdigit() and time[3:5].isdigit()) and (time[-4:] == "a.m." or time[-4:] == "p.m.")

def convert(time):
    if validate_12_hour_format(time):
        hours, minutes, period = time.split(":")[0], time.split(":")[1][:2], time.split(" ")[1]
