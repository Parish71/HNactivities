...
def validate_time_format(time):
    ...
    parts = time.split(":")
    return (
        len(time) == 4 or (len(time) == 5 and time[2] == ":")
    ) and parts[0].isdigit() and len(parts[0]) <= 2 and parts[1].isdigit() and len(parts[1]) == 2

def validate_12_hour_format(time):
    # Check if the input is in the format HH:MM AM/PM
    return (
        len(time) == 7 or (len(time) == 8 and time[2] == ":")
    ) and time[:2].isdigit() and len(time[:2]) <= 2 and time[3:5].isdigit() and len(time[3:5]) == 2 and (
        time[-4:] == "a.m." or time[-4:] == "p.m."
    )
